            ...
            <tbody>
                @foreach($uploads as $upload)
                    <tr>
                        ...
                        <td>{{ $upload->category->name ?? 'Không có danh mục' }}</td>
                        <td>
                            @if($upload->image)
                        ...
                        @php
                            ...
                            if (!empty($upload->type)) {
                                ...
                                if (str_contains($mime, 'pdf')) {
                                    $fileType = 'PDF';
                                } elseif (str_contains($mime, 'word') || str_contains($mime, 'msword') || str_contains($mime, 'officedocument.wordprocessingml')) {
                                    $fileType = 'Word';
                                } elseif (str_contains($mime, 'excel') || str_contains($mime, 'spreadsheet') || str_contains($mime, 'officedocument.spreadsheetml')) {
                                    $fileType = 'Excel';
                                } elseif (str_contains($mime, 'powerpoint') || str_contains($mime, 'presentation')) {
                                    $fileType = 'PowerPoint';
                                } elseif (str_contains($mime, 'image') || str_contains($mime, 'jpeg') || str_contains($mime, 'png')) {
                                    $fileType = 'Hình ảnh';
                                } else {
